...
DEFINE_HOOK(0x413F68, AircraftClass_CTOR, 0x6)
{
	GET(Proxy<AircraftClass>&, item, ESI);
...
{
	GET(Proxy<AircraftClass>&, pThis, ESI);

	auto& const pExt = pThis.extension();
...
{
	GET(Proxy<AircraftClass>&, pThis, EDI);
	GET(Proxy<AbstractClass>*, pTarget, EAX);
...
long __stdcall AircraftClass_IFlyControl_IsStrafe(IFlyControl const* ifly)
{
	...
	auto& const pThis = *reinterpret_cast<Proxy<AircraftClass> const*>(static_cast<AircraftClass const*>(ifly));
	auto& const pExt = pThis.extension();
...
DEFINE_HOOK(0x418403, AircraftClass_Mission_Attack_FireAtTarget_BurstFix, 0x8)
{
	...
	pThis.FireWeapon(pThis.Target, 0);
...
DEFINE_HOOK(0x418805, AircraftClass_Mission_Attack_FireAtTarget2Strafe_BurstFix, 0x8)
{
	...
	pThis.FireWeapon(pThis.Target, 1);
...
DEFINE_HOOK(0x418914, AircraftClass_Mission_Attack_FireAtTarget3Strafe_BurstFix, 0x8)
{
	...
	pThis.FireWeapon(pThis.Target, 2);
...
DEFINE_HOOK(0x418A23, AircraftClass_Mission_Attack_FireAtTarget4Strafe_BurstFix, 0x8)
{
	...
	pThis.FireWeapon(pThis.Target, 3);
...
DEFINE_HOOK(0x418B1F, AircraftClass_Mission_Attack_FireAtTarget5Strafe_BurstFix, 0x8)
{
	...
	pThis.FireWeapon(pThis.Target, 4);

	return 0x418B40;
}

WeaponStruct* __fastcall AircraftClass_GetWeapon_Wrapper(AircraftClass* pThis_, void* _, int weaponIndex)
{
	auto& const pThis = *static_cast<Proxy<AircraftClass>*>(pThis_);
	auto& const pExt = pThis.extension();
...
void __fastcall AircraftClass_SetTarget_Wrapper(AircraftClass* pThis_, void* _, AbstractClass* pTarget)
{
	auto& const pThis = *static_cast<Proxy<AircraftClass>*>(pThis_);
	auto& const pExt = pThis.extension();
...
DEFINE_HOOK(0x4CF31C, FlyLocomotionClass_FlightUpdate_LandingDir, 0x9)
{
	...
	{
		if (auto const pAircraft = static_cast<Proxy<AircraftClass>*>(abstract_cast<AircraftClass*>(pLinkedTo)))
			dir = DirStruct(pAircraft->GetLandingDir()).Raw;
	}
...
DirType _fastcall AircraftClass_PoseDir_Wrapper(AircraftClass* pThis_)
{
	auto& const pThis = *static_cast<Proxy<AircraftClass>*>(pThis_);
	return pThis.GetLandingDir(SeparateAircraftTemp::pBuilding);
}
...
DEFINE_HOOK(0x443FC7, BuildingClass_ExitObject_PoseDir1, 0x8)
{
	...
	GET(Proxy<AircraftClass>&, aircraft, EBP);

	R->EAX(aircraft.GetLandingDir(pThis));
...
DEFINE_HOOK(0x44402E, BuildingClass_ExitObject_PoseDir2, 0x5)
{
	...
	auto dir = DirStruct(aircraft.GetLandingDir(pThis));
	// aircraft.PrimaryFacing.SetCurrent(dir);
	aircraft.SecondaryFacing.SetCurrent(dir);
...
DEFINE_HOOK(0x4CF68D, FlyLocomotionClass_DrawMatrix_OnAirport, 0x5)
{
	...
	auto& pThis = *static_cast<Proxy<AircraftClass>*>(loco->LinkedTo);
