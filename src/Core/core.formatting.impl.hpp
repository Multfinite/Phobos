...
void parse<T, Allocate>(T& data, CCINIClass& ini, INI_EX& parser, std::string const& section, std::string const& prefix) \
{ \
	detail::read<T, Allocate>(data, parser, section.c_str(), prefix.c_str()); \
} \
template<bool Allocate = true> \
void parse<std::optional<T>, Allocate>(std::optional<T>& data, CCINIClass& ini, INI_EX& parser, std::string const& section, std::string const& prefix) \
{ \
	if (parser.ReadString(section.c_str(), prefix.c_str())) \
	{ \
